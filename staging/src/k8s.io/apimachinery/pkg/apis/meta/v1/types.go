...
//
// TypeMeta 使用表示对象类型及其 API 模式版本的字符串描述 API 响应或请求中的单个对象。版本化或持久化的结构应该内联 TypeMeta。
...
// ListMeta 描述了合成资源必须具备的元数据，包括列表和各种状态对象。资源可能只有 {ObjectMeta, ListMeta} 之一。
...
type ObjectMeta struct {
	...
	// definition.
	// 名称在命名空间内必须是唯一的。创建资源时需要，尽管某些资源可能允许客户端自动请求生成适当的名称。名称主要用于创建幂等性和配置定义。
	// Cannot be updated.
	...
	// GenerateName 是可选的前缀，仅当未提供 Name 字段时，服务器使用它来生成唯一的名称。如果使用此字段，则返回给客户端的名称将与传递的名称不同。
	// 该值还将与唯一后缀组合。提供的值具有与 Name 字段相同的验证规则，并且可能会被使该值在服务器上唯一所需的后缀长度截断。
	...
	// 名称空间定义了每个名称在其中必须唯一的空间。空命名空间等同于“默认”命名空间，但“默认”是规范表示。
	// 并非所有对象都需要限定在命名空间的范围内——这些对象的该字段的值将为空。
	// Must be a DNS_LABEL.
	// Cannot be updated.
	...
	// operations.
	// UID 是此对象在时间和空间上唯一的值。它通常由服务器在成功创建资源时生成，并且不允许在 PUT 操作中更改。
	//
	// Populated by the system.
	// Read-only.
	...
	// 一个不透明的值，表示此对象的内部版本，客户端可以使用它来确定对象何时更改。可用于乐观并发、变更检测以及对一个或一组资源的监视操作。
	// 客户端必须将这些值视为不透明的并且不加修改地传回服务器。它们可能仅对特定资源或资源集有效。
	// Populated by the system.
	// Read-only.
	...
	//
	// CreationTimestamp 是表示创建此对象时的服务器时间的时间戳。它不能保证在不同的操作中以 happens-before 顺序设置。
	// 客户端可能不会设置此值。它以 RFC3339 格式表示并采用 UTC。
	// Populated by the system.
	// Read-only.
	// Null for lists.
...
// APIVersions 列出了可用的版本，以允许客户端在 api 处发现 API，这是旧版 v1 API 的根路径。
//
...
// /apis.
// APIGroupList 是 APIGroup 的列表，以允许客户端在 api 上发现 API。
...
// is namespaced.
// APIResourceList 是APIResource 的列表，它用于暴露在特定组和版本中支持的资源的名称，以及资源是否命名空间。
...
type LabelSelector struct {
	...
	// +optional
	// matchLabels是一个{key,value}对的映射。 matchLabels映射中的单个{key,value}等效于matchExpressions的元素，其key字段为“key”，运算符为“In”，并且值数组仅包含“value”。要求是ANDed。
...
// 标签选择器需求是一个包含值的选择器，一个键和一个运算符，该运算符将键和值相关联。
type LabelSelectorRequirement struct {
	...
	// 运算符表示键与一组值的关系。
	//有效的运算符是In，NotIn，Exists和DoesNotExist。
	...
	// merge patch.
	// +optional
	// values是一个字符串值的数组。如果运算符是In或NotIn，则values数组必须非空。如果运算符是Exists或DoesNotExist，则values数组必须为空。在战略合并补丁期间，将替换此数组。
	Values []string `json:"values,omitempty" protobuf:"bytes,3,rep,name=values"`
}

// A label selector operator is the set of operators that can be used in a selector requirement.
// 标签选择器运算符是可以在选择器要求中使用的运算符集。
