...
const (
	...
	// logMonitorPeriod是容器日志管理器监视容器日志并执行日志轮换的周期。
...
// ContainerLogManager管理所有容器日志的生命周期。
// 实现是线程安全的。
...
// LogRotatePolicy是容器日志轮换的策略。该策略适用于kubelet管理的所有容器。
...
// GetAllLogs获取特定容器日志的所有正在使用的（旋转/压缩）日志。 返回的日志按最旧到最新的顺序排序。
...
// compressReadCloser将gzip.Reader封装为一个函数，以关闭文件处理程序。
...
// TODO(#59902): Leverage this function to support log rotation in `kubectl logs`.
// UncompressLog压缩压缩的日志，并返回未压缩内容的流的读取器。
...
// parseMaxSize将数量字符串解析为字节的int64最大大小。
...
// failure.
// cleanupUnusedLogs清理由以前的日志轮换失败生成的临时或未使用的日志文件。
...
// filterUnusedLogs将日志分成两组，第一组是使用中的日志，第二组是未使用的日志。
...
// removeExcessLogs删除旧日志，以确保只有最多MaxFiles个日志文件。
...
// rotateLatestLog旋转最新的日志，而不进行压缩，以便容器仍然可以写入并且fluentd可以完成读取。
